...
export default function ExpenseReports() {
  ...
  return (
    <div>
      ...
      {!report ? (
        <div className="glass-panel p-16 rounded-3xl text-center text-outline/50 font-label-caps text-xs">Loading report...</div>
      ) : (
        <>
          {/* Summary cards */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
            <div className="glass-panel p-6 rounded-2xl">
              <p className="font-label-caps text-[10px] text-outline mb-1">TOTAL RECORDS</p>
              <h3 className="font-headline-md text-2xl text-primary">{report.summary.totalCount}</h3>
            </div>
            <div className="glass-panel p-6 rounded-2xl">
              <p className="font-label-caps text-[10px] text-outline mb-1">PAID</p>
              <h3 className="font-headline-md text-2xl text-secondary">{formatCurrency(report.summary.paidAmount)}</h3>
            </div>
            <div className="glass-panel p-6 rounded-2xl">
              <p className="font-label-caps text-[10px] text-outline mb-1">PENDING</p>
              <h3 className="font-headline-md text-2xl text-primary">{formatCurrency(report.summary.pendingAmount)}</h3>
            </div>
            <div className="glass-panel p-6 rounded-2xl">
              <p className="font-label-caps text-[10px] text-outline mb-1">OVERDUE</p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10">
            ...
            <div className="glass-panel p-8 rounded-3xl border border-white/12">
              ...
              <div className="space-y-3">
                <div className="flex justify-between"><span className="font-label-caps text-[10px] text-outline">PAID</span><span className="font-data-tabular text-sm font-bold text-secondary">{formatCurrency(report.paidVsPending.paid)}</span></div>
                <div className="flex justify-between"><span className="font-label-caps text-[10px] text-outline">PENDING + OVERDUE</span><span className="font-data-tabular text-sm font-bold text-primary">{formatCurrency(report.paidVsPending.pending)}</span></div>
              </div>
            </div>
            ...
            <div className="glass-panel p-8 rounded-3xl border border-white/12">
              ...
              <div className="space-y-3">
                <div className="flex justify-between"><span className="font-label-caps text-[10px] text-outline">RECURRING</span><span className="font-data-tabular text-sm font-bold text-primary">{formatCurrency(report.recurringVsManual.recurring)}</span></div>
                <div className="flex justify-between"><span className="font-label-caps text-[10px] text-outline">MANUAL</span><span className="font-data-tabular text-sm font-bold text-primary">{formatCurrency(report.recurringVsManual.manual)}</span></div>
              </div>
            </div>
          </div>
          ...
          <div className="glass-panel p-8 rounded-3xl border border-white/12 mb-10">
            ...
            <p className="font-label-caps text-[9px] text-outline tracking-widest mb-4">BASED ON PAYMENT DATE, NOT DUE DATE</p>
            <div className="flex gap-8">
              <div>
                <p className="font-label-caps text-[10px] text-outline mb-1">TOTAL PAID OUT</p>
                <p className="font-headline-md text-2xl text-secondary">{formatCurrency(report.cashFlow.totalPaidOut)}</p>
              </div>
              <div>
                <p className="font-label-caps text-[10px] text-outline mb-1">PAYMENTS MADE</p>
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-primary/5">
                  ...
                  {report.rows.length === 0 && (
                    <tr>
                      <td colSpan={6} className="px-8 py-16 text-center text-outline/50 font-label-caps text-xs">
